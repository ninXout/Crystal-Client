...
@implementation BypassTimerCaller
	...
	- (void)timerFired:(id)ok {
		...
		auto intended_dt = 1.0 / getSavedVar<float>("FPS");
...
void setInterval(directorStructure* caller, void*, double a) {
	...
	caller->renderTimer = [NSTimer timerWithTimeInterval:1.0 / getSavedVar<float>("FPS")
...
$execute {
    ...
    geode::Mod::get()->addHook(
        reinterpret_cast<void*>(geode::base::get() + 0x1da750),
        &setInterval,
        "ccschedulercaller hook",
