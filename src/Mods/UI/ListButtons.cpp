...
class $modify(ModifiedSearchLayer, LevelSearchLayer) {
	virtual bool init(int p0) {
		...
		if (getSavedVar<bool>("demonlist_button") || getSavedVar<bool>("challengelist_button")) {
		...
		if (getSavedVar<bool>("demonlist_button")) {
		...
		if (getSavedVar<bool>("challengelist_button")) {
			auto menu = findFirstChildRecursive<CCLabelBMFont>(this, [](CCLabelBMFont* lbl) {
				return strncmp(lbl->getString(), "Trending", 8) == 0;
			})->getParent()->getParent()->getParent();
...
class $modify(GameLevelManager) {
	void ProcessHttpRequest(gd::string gdurl, gd::string gdquery, gd::string idk, GJHttpType type) {
		...
		if (url == "http://www.boomlings.com/database/getGJLevels21.php") {
			...
			if (query.find("type=3141") != std::string::npos && getSavedVar<bool>("demonlist_button")) {
				url = std::string("http://absolllute.com/api/mega_hack/demonlist/page") + std::to_string(thing) + ".txt";
			} else if (query.find("type=3142") != std::string::npos && getSavedVar<bool>("challengelist_button")) {
