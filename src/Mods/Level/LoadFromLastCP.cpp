...
struct LoadFromLastCP : Modify<LoadFromLastCP, EndLevelLayer> {
	...
	static EndLevelLayer* create() {
		...
		if (getSavedVar<bool>("load_from_last_CP") && PlayLayer::get()->m_isPracticeMode) {
			...
			*setTempVar<bool>("should_quit") = true;
			...
			button->setID("load-last-cp"_spr);
...
class $modify(PlayLayer) {
	...
	void onQuit() {
		if ((getSavedVar<bool>("load_from_last_CP") && getTempVar<bool>("should_quit")) || !getSavedVar<bool>("load_from_last_CP")) {
			PlayLayer::onQuit();
		}
	}
};